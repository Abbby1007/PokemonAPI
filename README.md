# PokemonAPI
In this project I used PokiApi to make a name and type Pokemon Quiz.
Just click the link and choose a quiz.

The demo link: https://abbby1007.github.io/PokemonAPI/

I made the game with the purpose of learning about APIs. When I watch on tutorial video I found myself needing to watch four other videos to understand different concepts. It was really fun to work with outside data and I look forward to watching other API videos.

Home Screen
<img width="958" height="475" alt="Screenshot 2026-06-22 111153" src="https://github.com/user-attachments/assets/913c46cd-324e-4852-9a25-f6c019a7461f" />

When User is wrong
<img width="958" height="473" alt="Screenshot 2026-06-22 111255" src="https://github.com/user-attachments/assets/c969b1d8-fb04-48b8-92f2-42da146e68dc" />

When user is correct
<img width="955" height="470" alt="Screenshot 2026-06-22 111535" src="https://github.com/user-attachments/assets/a78c6a16-2c0f-4377-9598-5583fc5509b1" />

When user finsihes the 10 questions
<img width="956" height="473" alt="Screenshot 2026-06-22 111912" src="https://github.com/user-attachments/assets/9cd9bb68-4aa5-4d16-9444-2a5413f2507c" />

